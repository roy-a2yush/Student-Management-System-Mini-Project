...
<?php
	...
	if(isset($_POST['FName']) && isset($_POST['LName']) && isset($_POST['phoneNo']) && isset($_POST['email']) && $_POST['Tid'] && isset($_POST['DepartmentAllotted']) && isset($_POST['pass']))
	{
		//entering
		$Fname=$_POST['FName'];
		$Lname=$_POST['LName'];
		...
		{
			$sql = "UPDATE `Student` SET `fname`='$Fname',`lname`='$Lname',`phoneNo`='$phoneNo',`email`='$email',`class`=$deptAlloted WHERE id = $Tid;";
			$result = mysqli_query($con,$sql);	
			$sql1 = "UPDATE `Marks` SET `class`=$deptAlloted WHERE Sid = $Tid";
